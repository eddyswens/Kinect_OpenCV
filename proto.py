...
def listen(ident, fields, fn):
    try:
        match = fields['id'] in ident
    except:
        match = fields['id'] == ident
    return fn(fields) if match else False
...
class NetworkStream:
    def __init__(self, networkAddress, networkPort, modemAddress=None, modemPort=None):
        try:
            verbose('Connecting to {:s}:{:d}, modem {:d}:{:d}'.format(networkAddress, networkPort,
...
class Field:
    ...
    def readInfo(self, callback=None, blocking=True):
        ...
        if not blocking or callback is not None:
            ...
            elif response['id'] == Message.COMPONENT_FIELD_ERROR:
...
class File:
    ...
    def readFileInfo(self, verify=False, callback=None, blocking=True):
        ...
        if not blocking or callback is not None:
            ...
            response = self.component.messenger.invoke(packet=request, callback=proxyCallback, retries = 4, timeout = 2)
    ...
    def readImpl(self, chunkSize, burstSize, verify):
        ...
        if state.total > 0:
            ...
            state.data = numpy.zeros(state.total, dtype=numpy.uint8)
            self.state = state
...
class Component:
    ...
    def readInfo(self, callback=None, blocking=True):
        ...
        if not blocking or callback is not None:
            ...
            if response['id'] == Message.COMPONENT_INFO_ERROR:
                raise CommandError(response['result'])
    ...
    def writeRadio(self, value, callback=None, blocking=True):
        request = {'id': Message.COMPONENT_RAW_DATA, 'component': self.address, 'payload': value}

        if not blocking or callback is not None:
            self.messenger.invokeAsync(packet=request)
        else:
            response = self.messenger.invoke(packet=request)
            #if response['result'] != Result.SUCCESS:
             #   raise CommandError(response['result'])
    # FIXME
...
class Hub:
    ...
    def connect(self):
        ...
        # lic = self.getLicenseInfo()
        # print(lic)
        # print(''.join(['%02x' % x for x in lic['uuid']]))
            ...
            try:
                ...
                if self.cache is not None:
                    try:
                        ...
                        descriptionData = open(cachedDescriptionName, 'rb').read()
                ...
                if not loadedFromCache and self.components[i].readFieldInfo():
                    ...
                    if self.cache is not None:
                        ...
                        open(cachedDescriptionName, 'wb').write(json.dumps(description).encode())
            except CommandError as e:
...
class Messenger:
    ...
    class GenericRequest:
        ...
        def genericHandler(messenger, packet, callback, timeout):
            def waitForCompletion(state, timeout):
                ...
                timer.start()
                state.sem.acquire()
                if (packet['id'] == 47 or packet['id'] == 60) and state.response is None:
                    state.sem.acquire()
                endTime = time.time()
                timer.cancel()
            ...
            # Second stage: is command was queued, wait for deferred answer
            try:
    ...
    def __init__(self, stream, cache=None):
        ...
        self.defaultTimeout = 0.55
        self.defaultWriteMult = 16
